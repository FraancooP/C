...
void push(struct node **,struct node **, struct Cargamento);
...
int main(int argc, char *argv[]) {
	...
	do{
		...
		case 'd':
		case 'D':
			cargamento_mayor(front);
			break;
		case 'e':
		case 'E':
			cargamento_menor(front);
			break;
		case 'f':
		case 'F':
			total_precio(front);
...
void push(struct node **front, struct node **back,struct Cargamento c){
...
void cargamento_mayor(struct node *front){
	struct node *temp=NULL;
	if(front==NULL){
...
void cargamento_menor(struct node *front){
	struct node *temp=NULL;
	if(front==NULL){
		printf("No hay cargamentos registrados.\n");
		return;
...
void total_precio(struct node *front){
	float total=0.0f;
	struct node *temp=NULL;
	if(front==NULL){
		printf("No hay cargamentos registrados.\n");
