...
el promedio de las 3 notas y en el caso de que tuvo un ausente o reprobó una 
instancia,
...
calificaciones.
El promedio debe ser calculado por una función. Una vez cargada la lista, mostrar
...
void pop(struct node **, struct Calificaciones);
...
int main(int argc, char *argv[]) {
	...
	do{
		...
		case 'e':
		case 'E':
			alumnos_reprobaron(head);
			break;
		case 'f':
		case 'F':
			alumnos_aprobaron(head);
			break;
		case 'g':
		case 'G':
			faltaron(head);
...
void push(struct node ** head, struct Calificaciones c){
...
void pop(struct node ** head, struct Calificaciones c){
	char dato[50];
	char borrar[50];
	...
	if(head==NULL){
		...
		if(strcmp(dato,(*head)->c.nombre_alumno) == 0){
			temp=*head;
			*head=(*head)->next;
			free(temp);
		}else{
			prev=*head;
			current=(*head)->next;
			while(current!=NULL and strcmp(current->c.nombre_alumno, dato) != 0){
				prev=current;
...
void mejor_alumno(struct node * head){
	struct node *temp=NULL;
...
void alumnos_reprobaron(struct node * head){
	struct node *temp=NULL;
	if(head==NULL){
		printf("No hay alumnos cargados.\n");
		return;
...
void alumnos_aprobaron(struct node * head){
	struct node *temp=NULL;
	if(head==NULL){
		printf("No hay alumnos cargados.\n");
		return;
...
void faltaron(struct node * head){
	struct node *temp=NULL;
	if(head==NULL){
		printf("No hay alumnos cargados.\n");
		return;
